...
def process_video(video_file, intensity, preview_audio=False, convert_vertical=False):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        video = video.set_audio(eq_path)

        output_path = os.path.join(tmpdir, "output.mp4")
        video.write_videofile(output_path, codec="libx264", audio_codec="aac", temp_audiofile="temp-audio.m4a", remove_temp=True, logger=None)
